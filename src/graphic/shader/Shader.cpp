	...
	: ID(0)
{
	...
	if (!success) {
		...
		glDeleteShader(ID);
		Console::Warn("Failed to compile shader.", { "Path: " + path, info_log });
	}
}
...
std::string Shader::load(const std::string& path)
{
	...
	try {
		...
		if (shader_file.is_open()) {
			...
			Console::Info("The shader file was read successfully.", { "Path: " + path });
		}
		else {
			Console::Info("Failed to open shader file.", { "Path: " + path });
		}
	}
	catch (std::ifstream::failure& exc) {
		Console::Info("Failed to read shader file.", { "Path: " + path });
	}
	return shader_code;
}